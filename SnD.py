...
def render_airtime_sales_content(data):
    ...
    with col_chart_right:
        ...
        try:
            ...
            # Fallback: Display simple bar chart
            st.warning("📍 Map requires 'gadm41_BEN_1.json' file. Displaying bar chart instead.")
            
            fig_regional = px.bar(
                regional_airtime.sort_values(selected_col, ascending=True),
                y='region',
                x=selected_col,
                orientation='h',
                color=selected_col,
            ...
            fig_regional.update_layout(
                ...
                **get_mtn_plotly_theme()['layout'],
                ...
                xaxis=dict(showgrid=True, gridcolor='#F0F0F0', title=airtime_map_kpi),
                yaxis=dict(showgrid=False, title='')
            )
...
def render_customer_conversion_content(data):
    ...
    # Daily Performance Table - Modified to show one KPI at a time with selector (inspired by acquisition page)
    st.markdown('<br><div class="chart-title">📋 Daily Performance - Conversion Metrics (Last 7 Days)</div>', unsafe_allow_html=True)
    
    # KPI Selector
    col_selector, col_empty = st.columns([2, 3])
    with col_selector:
        selected_conv_kpi = st.selectbox(
            "Select KPI to View",
            ["MoMo Conv.", "MoMo Rate", "Data Conv.", "Data Rate", "Bundle Rate", "Multi-Service"],
            index=0,
            key="conv_daily_kpi_selector"
        )
    
    # Generate daily performance data for last 7 days
    last_7_days = pd.date_range(end=datetime.now(), periods=7, freq='D')
    
    # KPI data mapping with realistic values (using KPIs from cards)
    kpi_values_map = {
        "MoMo Conv.": np.random.uniform(2500, 3500, 7),
        "MoMo Rate": np.random.uniform(18, 21, 7),
        "Data Conv.": np.random.uniform(3500, 4500, 7),
        "Data Rate": np.random.uniform(23, 27, 7),
        "Bundle Rate": np.random.uniform(32, 38, 7),
        "Multi-Service": np.random.uniform(11, 15, 7)
    }
    
    # Budget/target values (sample, adjust as needed)
    budget_map = {
        "MoMo Conv.": 3000,
        "MoMo Rate": 19.5,
        "Data Conv.": 4000,
    ...
    # Create DataFrame
    selected_values = kpi_values_map[selected_conv_kpi]
    budget_value = budget_map[selected_conv_kpi]
    
    # Calculate variances (as percentages)
    dod_values = [(selected_values[i] - selected_values[i-1]) / selected_values[i-1] * 100 if i > 0 else np.nan 
                  for i in range(len(selected_values))]
    wow_values = [np.random.uniform(-5, 15, 1)[0] for _ in range(7)]  # Sample WoW data
    mom_values = [np.random.uniform(-10, 20, 1)[0] for _ in range(7)]  # Sample MoM data
    ...
    daily_performance = pd.DataFrame({
        'Date': last_7_days.strftime('%Y-%m-%d'),
        selected_conv_kpi: selected_values,
        'DoD': dod_values,
        'WoW': wow_values,
        'MoM': mom_values,
        'vs Budget': vs_budget
    ...
    # Format the KPI value column
    fmt = format_map[selected_conv_kpi]
    if '%' in fmt:
        display_df[selected_conv_kpi] = display_df[selected_conv_kpi].apply(lambda x: f"{x:.1f}%")
    else:
        display_df[selected_conv_kpi] = display_df[selected_conv_kpi].apply(lambda x: f"{x:,.0f}")
    
    # Format the variance columns with +/- signs
    for col in ['DoD', 'WoW', 'MoM', 'vs Budget']:
        display_df[col] = display_df[col].apply(lambda x: f"{x:+.1f}%" if pd.notna(x) else "-")
    
    # Color function for variance columns (matching acquisition dashboard)
    def get_color_for_percentage(value):
        """Return color based on percentage value"""
        if value >= 5:
            return '#107C10'  # Green
...
def main():
    ...
    with col3:
        # Use hardcoded date for better alignment
        current_date = datetime.now().strftime("%Y/%m/%d")
        st.markdown(f"""
        <div style="background: #F8F8F8; padding: 12px 20px 16px 20px; height: 100px; display: flex; flex-direction: column; justify-content: center; align-items: center; border-left: 1px solid #E0E0E0; border-bottom: 2px solid #E0E0E0;">
            <div style="color: #4A4A4A; font-size: 12px; font-weight: 600; margin-bottom: 8px;">Report Date</div>
            <div style="background: white; border: 1px solid #D0D0D0; border-radius: 3px; padding: 6px 16px; font-size: 14px; color: #1A1A1A; font-weight: 500;">
                {current_date}
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col_filter:
        ...
        # Region Filter
         ...
         # Region Filter - Benin Administrative Hierarchy
        st.markdown('<div class="filter-section">', unsafe_allow_html=True)
        st.markdown('<div class="filter-label">🗺️ Region</div>', unsafe_allow_html=True)
        
        # Level 1: Region (Main regions of Benin)
        ...
        # Level 2: Département (12 departments in Benin)
        departement_options = ["All Départements"]
        if selected_region == "North":
            departement_options += ["Alibori", "Atakora", "Borgou", "Donga"]
        elif selected_region == "Center":
            departement_options += ["Collines", "Zou"]
        elif selected_region == "South":
            departement_options += ["Atlantique", "Couffo", "Littoral", "Mono", "Ouémé", "Plateau"]
        else:
            departement_options += ["Alibori", "Atakora", "Atlantique", "Borgou", "Collines", 
        ...
        selected_departement = st.selectbox(
            ...
            departement_options,
            label_visibility="visible",
            key="departement_filter"
        )
        
        # Level 3: Commune (real communes by département)
        commune_options = ["All Communes"]
        if selected_departement == "Alibori":
            commune_options += ["Banikoara", "Gogounou", "Kandi", "Karimama", "Malanville", "Ségbana"]
        elif selected_departement == "Atakora":
            commune_options += ["Boukoumbé", "Cobly", "Kérou", "Kouandé", "Matéri", "Natitingou", "Péhunco", "Tanguiéta"]
        elif selected_departement == "Atlantique":
            commune_options += ["Abomey-Calavi", "Allada", "Kpomassè", "Ouidah", "Sô-Ava", "Toffo", "Tori-Bossito", "Zè"]
        elif selected_departement == "Borgou":
            commune_options += ["Bembéréké", "Kalalé", "N'Dali", "Nikki", "Parakou", "Pèrèrè", "Sinendé", "Tchaourou"]
        elif selected_departement == "Collines":
            commune_options += ["Bantè", "Dassa-Zoumè", "Glazoué", "Ouèssè", "Savalou", "Savè"]
        elif selected_departement == "Couffo":
            commune_options += ["Aplahoué", "Djakotomey", "Dogbo", "Klouékanmè", "Lalo", "Toviklin"]
        elif selected_departement == "Donga":
            commune_options += ["Bassila", "Copargo", "Djougou", "Ouaké"]
        elif selected_departement == "Littoral":
            commune_options += ["Cotonou"]
        elif selected_departement == "Mono":
            commune_options += ["Athiémé", "Bopa", "Comè", "Grand-Popo", "Houéyogbé"]
        elif selected_departement == "Ouémé":
            commune_options += ["Adjarra", "Adjohoun", "Aguégués", "Akpro-Missérété", "Avrankou", "Bonou", "Dangbo", "Porto-Novo", "Sèmè-Kpodji"]
        elif selected_departement == "Plateau":
            commune_options += ["Adja-Ouèrè", "Ifangni", "Kétou", "Pobè", "Sakété"]
        elif selected_departement == "Zou":
            commune_options += ["Abomey", "Agbangnizoun", "Bohicon", "Cové", "Djidja", "Ouinhi", "Za-Kpota", "Zagnanado", "Zogbodomey"]
        ...
        # Level 4: Arrondissement (real arrondissements by commune)
        arrondissement_options = ["All Arrondissements"]
        
        # Alibori arrondissements
        if selected_commune == "Banikoara":
            arrondissement_options += ["Banikoara I", "Banikoara II", "Founougo", "Gomparou", "Goumori", "Kokiborou", "Kokey", "Ounet", "Sori", "Toura"]
        elif selected_commune == "Kandi":
            arrondissement_options += ["Kandi I", "Kandi II", "Kandi III", "Angaradébou", "Bensékou", "Donwari", "Kassakou", "Sam", "Sonsoro"]
        elif selected_commune == "Malanville":
            arrondissement_options += ["Malanville", "Garou", "Guéné", "Madécali", "Toumboutou"]
        
        # Atakora arrondissements
        elif selected_commune == "Natitingou":
            arrondissement_options += ["Natitingou I", "Natitingou II", "Natitingou III", "Kotopounga", "Perma", "Tchatcha"]
        elif selected_commune == "Boukoumbé":
            arrondissement_options += ["Boukoumbé", "Dipoli", "Korontière", "Kossoucoingou", "Manta", "Natta"]
        
        # Atlantique arrondissements
        elif selected_commune == "Abomey-Calavi":
            arrondissement_options += ["Akassato", "Calavi", "Godomey", "Hêvié", "Kpanroun", "Ouèdo", "Togba"]
        elif selected_commune == "Allada":
            arrondissement_options += ["Allada", "Attogon", "Ayou", "Hinvi", "Lon-Agonmey", "Sékou", "Togoudo"]
        elif selected_commune == "Ouidah":
            arrondissement_options += ["Ouidah I", "Ouidah II", "Ouidah III", "Ouidah IV", "Pahou", "Savi"]
        
        # Borgou arrondissements
        elif selected_commune == "Parakou":
            arrondissement_options += ["Parakou I", "Parakou II", "Parakou III", "Guinagourou"]
        elif selected_commune == "Nikki":
            arrondissement_options += ["Nikki", "Biro", "Ouénou", "Tasso"]
        
        # Collines arrondissements
        elif selected_commune == "Savalou":
            arrondissement_options += ["Savalou", "Djalloukou", "Gobada", "Logozohe", "Ouesse"]
        elif selected_commune == "Dassa-Zoumè":
            arrondissement_options += ["Dassa-Zoumè", "Kéré-Kéré", "Lèma", "Paouignan"]
        elif selected_commune == "Glazoué":
            arrondissement_options += ["Glazoué", "Kpakpassa", "Magoumi", "Ouèdèmè", "Sokponta"]
        
        # Couffo arrondissements
        elif selected_commune == "Aplahoué":
            arrondissement_options += ["Aplahoué", "Atomey", "Azovè", "Dekpo", "Gohomey", "Lonkly"]
        elif selected_commune == "Dogbo":
            arrondissement_options += ["Dogbo", "Ayomi", "Lokogohoué", "Madjrè"]
        
        # Donga arrondissements
        elif selected_commune == "Djougou":
            arrondissement_options += ["Djougou I", "Djougou II", "Djougou III", "Barei"]
        
        # Littoral arrondissements (Cotonou's 13 arrondissements)
        elif selected_commune == "Cotonou":
            arrondissement_options += ["Cotonou 1er", "Cotonou 2ème", "Cotonou 3ème", "Cotonou 4ème", 
                                      "Cotonou 5ème", "Cotonou 6ème", "Cotonou 7ème", "Cotonou 8ème",
                                      "Cotonou 9ème", "Cotonou 10ème", "Cotonou 11ème", "Cotonou 12ème", "Cotonou 13ème"]
        
        # Mono arrondissements
        elif selected_commune == "Grand-Popo":
            arrondissement_options += ["Grand-Popo", "Agoué", "Comé", "Sè"]
        elif selected_commune == "Comè":
            arrondissement_options += ["Comè", "Akodéha", "Ouedeme-Pedah"]
        
        # Ouémé arrondissements
        elif selected_commune == "Porto-Novo":
            arrondissement_options += ["Porto-Novo 1er", "Porto-Novo 2ème", "Porto-Novo 3ème", "Porto-Novo 4ème", "Porto-Novo 5ème"]
        elif selected_commune == "Sèmè-Kpodji":
            arrondissement_options += ["Sèmè-Kpodji", "Agblangandan", "Djeffa", "Ekpè", "Tohouè"]
        elif selected_commune == "Adjarra":
            arrondissement_options += ["Adjarra", "Aglogbé", "Malanhoui"]
        
        # Plateau arrondissements
        elif selected_commune == "Kétou":
            arrondissement_options += ["Kétou", "Idigny", "Issaba", "Okpometa"]
        elif selected_commune == "Pobè":
            arrondissement_options += ["Pobè", "Aguidi", "Igolo", "Towé"]
        elif selected_commune == "Sakété":
            arrondissement_options += ["Sakété", "Akiza", "Ita-Djèbou", "Takon"]
        
        # Zou arrondissements
        elif selected_commune == "Abomey":
            arrondissement_options += ["Abomey", "Djègbé", "Zoungoudo"]
        elif selected_commune == "Bohicon":
            arrondissement_options += ["Bohicon", "Avogbana", "Lissèzoun", "Ouanho", "Sodohomè"]
        elif selected_commune == "Cové":
            arrondissement_options += ["Cové", "Houin-Agouagon", "Koussoukpa", "Zoungoudo"]
        elif selected_commune == "Zogbodomey":
            arrondissement_options += ["Zogbodomey", "Avlamè", "Cana", "Kpokissa"]
        
        selected_arrondissement = st.selectbox(
            "Arrondissement",
            arrondissement_options,
            label_visibility="visible",
            key="arrondissement_filter"
        )
